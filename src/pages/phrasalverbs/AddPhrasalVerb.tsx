...
export default function AddPhrasalVerb() {
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault()
    const trimmed = phrasalVerb.trim()
    if (trimmed === '') return
    ...
    try {
      ...
      // New phrasal verbs are automatically starred for daily revision.
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10">
      ...
      <h1 className="mt-2 text-3xl font-extrabold text-slate-900">Add a Phrasal Verb</h1>
      <p className="mt-2 text-slate-500">
        Type a phrasal verb — we'll look up its meaning and an example sentence and add it to the shared phrasal verb
        bank for everyone, for good.
      </p>
      ...
      <form onSubmit={handleSubmit} className="mt-8 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        ...
        <button
          type="submit"
          disabled={status === 'loading' || phrasalVerb.trim() === ''}
      ...
      {status === 'success' && added && (
        <div className="mt-6 rounded-3xl bg-emerald-50 p-6 ring-1 ring-emerald-200">
          <p className="text-sm font-semibold text-emerald-700">Added to the shared phrasal verb bank for everyone — and starred for daily revision!</p>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-4">
            <Link to="/english/phrasal-verbs/library" className="text-sm font-semibold text-indigo-600 hover:underline">
              View in library →
            </Link>
            <Link to="/english/phrasal-verbs/quiz" className="text-sm font-semibold text-indigo-600 hover:underline">
              Play the quiz →
            </Link>
          </div>
        </div>
