...
import type { TwinProductQuestion } from '../lib/generators'
...
interface TwinProductQuizRunnerProps {
  gameId: string
  questions: TwinProductQuestion[]
  onExit: () => void
  onRestart: () => void
}

/** Both question kinds expect a plain positive integer, gated on digit count like validateInteger. */
function checkTwinProduct(raw: string, question: TwinProductQuestion): CheckResult {
  const trimmed = raw.trim()
  if (trimmed === '' || !/^\d+$/.test(trimmed)) return 'incomplete'
  if (trimmed.length < question.answer.length) return 'incomplete'
  return trimmed === question.answer ? 'correct' : 'wrong'
}

export default function TwinProductQuizRunner({ gameId, questions, onExit, onRestart }: TwinProductQuizRunnerProps) {
  ...
  function submit(raw: string) {
    ...
    const result = checkTwinProduct(raw, current)
    if (result === 'correct') {
      setFeedback('correct')
      setCorrect((c) => c + 1)
      // 'start' questions only ask for one of the two numbers, so confirm the full equation
      // before moving on rather than instantly advancing.
    ...
    <div className="mx-auto max-w-md px-4 py-6">
      ...
      >
        <div className="font-mono text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">
          {current.prompt}
          {current.kind === 'product' && <span className="text-slate-300"> = </span>}
        </div>
        {current.kind === 'start' && revealAnswer === null && (
          <p className="mt-2 text-sm text-slate-400">Enter the starting number</p>
        )}
        ...
        {revealAnswer !== null ? (
          ...
          <input
            ...
            autoCorrect="off"
            spellCheck={false}
            placeholder={current.kind === 'product' ? 'e.g. 20' : 'e.g. 7'}
            className={[
