...
import type { AlphaNumQuestion } from '../lib/generators'
...
interface AlphaNumQuizRunnerProps {
  gameId: string
  questions: AlphaNumQuestion[]
  onExit: () => void
  onRestart: () => void
}

/**
 * Checks a typed answer against the question's target — a single letter for num->alpha
 * questions, or a (possibly negative) number for alpha->num ones. For numbers, waits until
 * as many digits are typed as the target has before judging, so "-2" on the way to "-24"
 * isn't flagged wrong the instant it forms a valid but different number — the same digit-count
 * gate used elsewhere (validateInteger, RatioQuizRunner) for the same reason.
 */
function checkAlphaNum(raw: string, question: AlphaNumQuestion): CheckResult {
  const trimmed = raw.trim()
  if (trimmed === '') return 'incomplete'

  if (question.kind === 'num-to-alpha') {
    return trimmed.toUpperCase() === question.answer ? 'correct' : 'wrong'
  }

  if (!/^-?\d+$/.test(trimmed)) return 'incomplete'
  const expectedDigits = question.answer.replace('-', '').length
  const typedDigits = trimmed.replace('-', '').length
  if (typedDigits < expectedDigits) return 'incomplete'
  return trimmed === question.answer ? 'correct' : 'wrong'
}

export default function AlphaNumQuizRunner({ gameId, questions, onExit, onRestart }: AlphaNumQuizRunnerProps) {
  ...
  function submit(raw: string) {
    ...
    const result = checkAlphaNum(raw, current)
    if (result === 'correct') {
      setFeedback('correct')
      setCorrect((c) => c + 1)
  ...
  function handleChange(raw: string) {
    if (feedback !== 'idle' || revealAnswer !== null) return
    const cleaned = current.kind === 'num-to-alpha' ? raw.replace(/[^a-zA-Z]/g, '').slice(0, 1).toUpperCase() : raw.replace(/[^0-9-]/g, '')
    setInput(cleaned)
    submit(cleaned)
  }
      ...
      >
        ...
        {revealAnswer !== null ? (
          ...
          <input
            ...
            autoCorrect="off"
            spellCheck={false}
            placeholder={current.kind === 'num-to-alpha' ? 'A-Z' : 'e.g. -24'}
            className={[
              'mt-6 w-40 rounded-xl border-2 bg-slate-50 px-4 py-3 text-center font-mono text-3xl font-bold uppercase text-slate-900 outline-none transition-colors',
