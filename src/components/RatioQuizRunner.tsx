...
import type { RatioAnswerQuestion } from '../lib/generators'
...
interface RatioQuizRunnerProps {
  gameId: string
  questions: RatioAnswerQuestion[]
  onExit: () => void
  onRestart: () => void
}

/**
 * Checks a typed "n/d" answer against the target fraction, requiring the exact reduced form.
 * Waits until both parts have at least as many digits as the target before judging, so typing
 * "1/1" on the way to "1/10" isn't flagged wrong the instant it forms a valid (but different)
 * fraction — mirroring validateInteger's digit-count gate for the same reason.
 */
function checkRatio(raw: string, question: RatioAnswerQuestion): CheckResult {
  const trimmed = raw.trim()
  if (trimmed === '') return 'incomplete'
  const match = trimmed.match(/^(\d+)\/(\d+)$/)
  if (!match) return 'incomplete'
  if (match[1].length < String(question.numerator).length) return 'incomplete'
  if (match[2].length < String(question.denominator).length) return 'incomplete'
  const n = Number(match[1])
  const d = Number(match[2])
  if (n === question.numerator && d === question.denominator) return 'correct'
  return 'wrong'
}

export default function RatioQuizRunner({ gameId, questions, onExit, onRestart }: RatioQuizRunnerProps) {
  ...
  function handleChange(raw: string) {
    ...
    setInput(cleaned)
    const result = checkRatio(cleaned, current)
    if (result === 'correct') {
      setFeedback('correct')
      setCorrect((c) => c + 1)
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    const result = checkRatio(input, current)
    if (result === 'correct') {
      setFeedback('correct')
      setCorrect((c) => c + 1)
      setTimeout(advance, 260)
